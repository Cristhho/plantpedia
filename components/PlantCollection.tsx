    ...
    <Link href={`/entry/${slug}`}>
      <a title={`Go to ${plantName}`}>
        <div className="opacity-95 hover:opacity-100">
          <Image src={image.url} layout='intrinsic' width={460} aspectRatio='4:3' />
          <div className="p-4">
  ...
  return (
    <Link href={`/entry/${slug}`}>
      <a title={`Go to ${plantName}`}>
        ...
        >
          <Image src={image.url} layout='fixed' width={84} aspectRatio='1:1' fit='fill' className="flex-none" />
...
export function PlantEntryVertical({
  ...
  return (
    <div className="opacity-95 hover:opacity-100">
      <Link href={`/entry/${slug}`}>
        <a title={`Go to ${plantName}`}>
          <Image src={image.url} width={624} layout='intrinsic' aspectRatio='9:12' />
